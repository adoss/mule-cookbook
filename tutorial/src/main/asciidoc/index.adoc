...
Before starting our {tutorial} service, let's create a Hello World Connector, and review its generated code.

To create a Hello World connector:
...
To solve this, DevKit provides a pair of annotations that let you
modularize in different classes:

* The behavior you expose to users, using the @Connector annotation.
* Code related to the connection using @ConnectionStrategy. When you mark a field with @ConnectionStrategy, DevKit ensures that you have an initialized object set when the Mule app runs and that requests are made to your connector.

If there is no need for authentication and/or connection management, but you need users to configure parameters, mark your connection strategy class with @Configuration.
...
+
Add our client dependency so that we can use it in our connector. This way we can use a Java API built to connect to the {tutorial}.
+
...
. On the @Configuration ConnectionStrategy add a <<configurable,configurable>> field for the address were the {tutorial} service is hosted:
+
[source,java]
----
include::{resourcesDir}/java/v1/ConnectorConnectionStrategy.java[lines=20..25]
----
+
. On your Connector use the @Start on a method to initialize the client.
+
...
----
+
Check the full link:{resourcesDir}/java/v1/CookBookConnector.java[Connector] and link:{resourcesDir}/java/v1/ConnectorConnectionStrategy.java[Strategy] source code.
